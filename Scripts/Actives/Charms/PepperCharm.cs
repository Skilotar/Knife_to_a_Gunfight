...
namespace Knives
{
    class PepperCharm : PlayerItem
    {

        public static void Register()
        {
            //The name of the item
            string itemName = "Pepper Charm";

            //Refers to an embedded png in the project. Make sure to embed your resources! Google it
            string resourceName = "Knives/Resources/Charm/Pepper_Charm";
            resource = resourceName;
            //Create new GameObject
            GameObject obj = new GameObject(itemName);

            //Add a PassiveItem component to the object
            var item = obj.AddComponent<PepperCharm>();

            //Adds a sprite component to the object and adds your texture to the item sprite collection
            ItemBuilder.AddSpriteToObject(itemName, resourceName, obj);

            //Ammonomicon entry variables
            string shortDesc = "Too Hot!";
            string longDesc = "Attaches to your current gun. Only one charm can be equipped to a gun.\n" +
                "Adds high chance for fire.\n\n" +
                "" +
                "An ornamental gun charm adorned with an actual pepper. Don't eat it.." +
        ...
        public override void DoEffect(PlayerController user)
        {
            if (user.CurrentGun.GetComponent<MasterCharmComp>() == null)
            {
                ...
                charm.type = MasterCharmComp.CharmType.Pepper;
