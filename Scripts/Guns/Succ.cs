...
namespace Knives
{
    ...
    class Succ : AdvancedGunBehaviour
    {
        public static void Add()
        {
            ...
            gun.SetShortDescription("Some Bunnys Worst Nightmare");
            gun.SetLongDescription(
                "Suck Up enemies and fire air on reload. Air damage is based on health of enemies sucked.\n\n" +
                "" +
        ...
        {   // outer pull
            ...
            player.StartCoroutine(this.HandleSwing(player, vector, .2f, 3.6f));
        ...
        private void ProcessEnemy(AIActor target)
        {
            ...
            bool health_check = target.CompanionOwner != player && !target.healthHaver.IsBoss && (target.healthHaver.GetCurrentHealth() <= target.healthHaver.GetMaxHealth() * .5);
        ...
        public override void OnReloadPressed(PlayerController player, Gun gun, bool bSOMETHING)
        {
            ...
            try
            {
                ...
                {
                    StartCoroutine(BurstAir(player));
        ...
        private IEnumerator BurstAir(PlayerController player)
        {
            int count = succ.Count -1;
            for (int i = 0; i <= count ; i++)
            {
                AIActor orLoadByGuid = EnemyDatabase.GetOrLoadByGuid(succ[i]);
